...
import de.tud.cs.qpe.editors.incidence.view.figure.ColorRefFigure;
...
public class ColorRefEditPart extends AbstractGraphicalEditPart implements
	...
	protected IFigure createFigure() {
		...
		RGB realColor = ColorHelper.getRGBFromString(color.attributeValue(
				"real-color", "#666666"));
	...
	protected List getModelSourceConnections() {
		if(this.getParent() instanceof PlaceEditPart) {
			...
			if(parentPlaceEditPart.getType() == PlaceFigure.TYPE_INPUT_PLACE) {
				...
				// incidence-function.
				List<Element> sourceConnections = new ArrayList<Element>();
	...
	protected List getModelTargetConnections() {
		// Check the ColorRef's parent. If it is a IncommingPlace, then this method should return null.
		if(this.getParent() instanceof PlaceEditPart) {
			PlaceEditPart parentPlaceEditPart = (PlaceEditPart) this.getParent();
			if(parentPlaceEditPart.getType() == PlaceFigure.TYPE_OUTPUT_PLACE) {
				...
				// choose only thise who's target is in the current
				// incidence-function.
				List<Element> targetConnections = new ArrayList<Element>();
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if ("color-ref".equals(((Element) evt.getOldValue()).getName())
				&& DocumentManager.PROP_ATTRIBUTE_MODIFIED.equals(prop)) {
			view.setName(getCastedModel().attributeValue("name", "new color"));
