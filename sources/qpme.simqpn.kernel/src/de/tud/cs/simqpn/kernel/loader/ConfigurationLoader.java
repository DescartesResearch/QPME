...
public class ConfigurationLoader {
	...
	public SimQPNConfiguration configureSimulatorSettings(Element netXML, String configurationString) throws SimQPNException {
		log.debug("/////////////////////////////////////////////");
		...
		if (Integer.parseInt(simulatorSettings.attributeValue("scenario")) != 1 && configuration.stoppingRule != SimQPNConfiguration.FIXEDLEN)  {
			log.error(formatDetailMessage(
		...
		configuration.totRunLen = Double.parseDouble(simulatorSettings.attributeValue("total-run-length"));
		...
		if (Integer.parseInt(simulatorSettings.attributeValue("scenario")) != 3) {		
			...
			configuration.rampUpLen = Double.parseDouble(simulatorSettings.attributeValue("ramp-up-length"));
			log.debug("rampUpLen = " + configuration.rampUpLen + ";");			
		} else { // Method of Welch
			configuration.rampUpLen = configuration.totRunLen; // Note: The method of Welch is currently run until rampUpLen is reached.
		}
		...
		if(configuration.stoppingRule != SimQPNConfiguration.FIXEDLEN) {
			...
			configuration.timeBtwChkStops = Double.parseDouble(simulatorSettings.attributeValue("time-between-stop-checks"));
			...
			configuration.secondsBtwChkStops = Double.parseDouble(simulatorSettings.attributeValue("seconds-between-stop-checks"));
			log.debug("secondsBtwChkStops = " + configuration.secondsBtwChkStops + ";");
		}
		...
		*/
		...
		if (configuration.getAnalMethod() != SimQPNConfiguration.BATCH_MEANS && configuration.stoppingRule != SimQPNConfiguration.FIXEDLEN) {
			log.error(formatDetailMessage(
	 ...
	 */
	public SimQPNConfiguration configureBatchMeansMethod(Element netXML, SimQPNController sim) throws SimQPNException {
		/*
		...
		if (sim.configuration.getAnalMethod() == SimQPNConfiguration.BATCH_MEANS)  {
			...
			Iterator<Element> placeIterator; placeIterator = placeList.iterator();
			for (int p = 0; placeIterator.hasNext(); p++) {
				Element place = placeIterator.next();
				Place pl = sim.getNet().getPlace(p);
			/**
			for (int p = 0; p<sim.getNet().getNumPlaces(); p++) {
				Place pl = sim.getNet().getPlace(p);
			*/
				if (pl.statsLevel >= 3) {
					log.debug("places[" + p + "]");
					...
					for (int cr = 0; colorRefIterator.hasNext(); cr++) {
						...
						Element colorRefSettings = getSettings(colorRef, sim.getNet().getConfiguration());
						...
						if (colorRefSettings != null) {
							if (colorRefSettings.attributeValue("signLev") == null) {
								log.error(formatDetailMessage(
										...
										"configuration", sim.getNet().getConfiguration(),						
			...
			for (int p = 0; probeIterator.hasNext(); p++) {
				...
				if (pr.statsLevel >= 3) {
					log.debug("probes[" + p + "]");
					xpathSelector = XMLHelper.createXPath("color-refs/color-ref");
					Iterator<Element> colorRefIterator = xpathSelector.selectNodes(probe).iterator();
					for (int cr = 0; colorRefIterator.hasNext(); cr++) {
						Element colorRef = colorRefIterator.next();
						Element colorRefSettings = getSettings(colorRef, sim.getNet().getConfiguration());
						// Initialize Probe
	...
	public static Element getSettings(Element element, String configurationName) {
		XPath xpathSelector = XMLHelper.createXPath("meta-attributes/meta-attribute[starts-with(@xsi:type, 'simqpn') and @configuration-name='" + configurationName + "']");
