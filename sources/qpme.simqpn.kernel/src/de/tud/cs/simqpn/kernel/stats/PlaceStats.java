...
public class PlaceStats extends Stats implements java.io.Serializable {
	 ...
	 * @exception
	 */
	public void finish(int[] tokenPop, Executor executor) throws SimQPNException  {
		if (statsLevel >= 2)  //NOTE: This makes sure areaTkPop, areaTkColOcp and areaTkOcp (and areaQueUtilQPl for QPlaceQueueStats) are complete!
			for (int c = 0; c < numColors; c++)
				updateTkPopStats(c, tokenPop[c], 0, executor.getClock());
	 ...
	 * 
	 */
	public void processStats(Executor executor) throws SimQPNException {
		...
		for (int c = 0; c < numColors; c++) {
			...
			if (statsLevel >= 3) {
				...
				stDevST[c] = Math.sqrt(Descriptive.sampleVariance(numST[c],
						sumST[c], sumSqST[c]));
				if (executor.getConfiguration().getAnalMethod() == SimQPNConfiguration.BATCH_MEANS && minBatches[c] > 0) {
	...
	public void printReport(SimQPNConfiguration configuration) throws SimQPNException {
		...
		for (int c = 0; c < numColors; c++) {
			...
			printColorSojournTimeStats(report, c, configuration);
		}
	 ...
	 * @param c color index
	 */
	protected void printColorSojournTimeStats(StringBuilder report, int c, SimQPNConfiguration configuration) {
		if (statsLevel >= 3) {
			report.append("-----\n");
	...
	/* ADDED BY CDUTZ */
	public void addReportMetaData(Element element, SimQPNConfiguration configuration) {
