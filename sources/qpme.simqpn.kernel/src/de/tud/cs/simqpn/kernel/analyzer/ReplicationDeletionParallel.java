...
public class ReplicationDeletionParallel implements Analyzer {
	 ...
	 * @exception
	 */
	private static AggregateStats[] runMultRepl(Net net,
	...
	private static AggregateStats[] initStatsArray(Net net,
		 ...
		 * color c, set aggrStats[p * 2].signLevAvgST[c] } else { // QUEUEING
		 * PLACE For every color c, set aggrStats[p * 2].signLevAvgST[c] // -
		 * QUEUE For every color c, set aggrStats[(p * 2) + 1].signLevAvgST[c]
		 * // - DEPOSITORY }
		 */
		// Iterate through all places.
		/**
		 * NEW for(int p = 0; p<net.getNumPlaces(); p++){ pl = net.getPlace(p);
		 */
		...
		for (int p = 0; placeIterator.hasNext(); p++) {
			...
			if (pl.statsLevel >= 3) {
				...
				for (int c = 0; colorRefIterator.hasNext(); c++) {
					...
					if (pl instanceof QPlace) {
						if (element == null
								|| element.attributeValue("queueSignLevAvgST") == null) {
						...
						log.debug("aggrStats[" + p * 2 + "].signLevAvgST[" + c
