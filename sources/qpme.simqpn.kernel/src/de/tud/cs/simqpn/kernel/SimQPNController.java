...
public class SimQPNController {
	 ...
	 */
	public SimQPNController(Element netXML, String configurationName) throws SimQPNException {
	...
	public static void configure(Element netElement, String configurationString, String logConfigFilename) throws SimQPNException {
		...
		if (logConfigFilename != null) {
			...
			XPath xpathSelector = XMLHelper.createXPath("/net/meta-attributes/meta-attribute[@xsi:type = 'simqpn-configuration' and @configuration-name = '" + configurationString + "']/@output-directory");
			Attribute outputDirAttribute = (Attribute) xpathSelector.selectSingleNode(netElement);
			try {
				LogUtil.configureDefaultLogging(outputDirAttribute.getStringValue(), "SimQPN_Output_" + configurationString);
			} catch (IOException e) {
		...
		Element simulatorSettings = ConfigurationLoader.getSettings(netElement, configurationString);
		...
		SimQPNConfiguration.setDebugLevel(Integer.parseInt(simulatorSettings.attributeValue("verbosity-level")));
		...
		switch (Integer.parseInt(simulatorSettings.attributeValue("scenario", "-1"))) {
			...
			case 2: {
				...
				configuration.setNumRuns(Integer.parseInt(simulatorSettings.attributeValue("number-of-runs")));
		...
		configuration.setStatsDir(simulatorSettings.attributeValue("output-directory"));
