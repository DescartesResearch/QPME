...
public class Probe {
	 ...
	 */
	public Probe(Probe probe, SimQPNConfiguration configuration, Place[] places) throws SimQPNException {
		this.id = probe.id;
		this.xmlId = probe.xmlId;
		this.name = probe.name;
		this.colors = probe.colors.clone();
		this.startPlace = places[probe.startPlace.id];
		this.startTrigger = probe.startTrigger;
		this.endPlace = places[probe.endPlace.id];
		this.endTrigger = probe.endTrigger;
	...
	public Probe(int id, String xmlId, String name, String[] colors,
			Place startPlace, int startTrigger, Place endPlace, int endTrigger,
			int statsLevel, Element element, SimQPNConfiguration configuration)
			throws SimQPNException {
		this.id = id;
		this.xmlId = xmlId;
		this.name = name;
	...
	public void instrument() throws SimQPNException {
		if (statsLevel >= 3) {
			...
			int result;
			if ((startPlace == endPlace) && (startTrigger == ON_ENTRY)
					&& (endTrigger == ON_EXIT)) {
				...
				// List of all indeterminate places. Indeterminate places are
				// places
				// for which it has not been determined yet, whether it is
				// possible
				// to reach the target place (-> start place). At the beginning
				// only
				// the end place is in this list.
				List<Place> indeterminatePlaces = new LinkedList<Place>();
				indeterminatePlaces.add(endPlace);

				// Assumption: For all places it can be determined, whether the
				// target
				// place is reachable. However, it might require several
				// attempts,
				// if cycles need to be resolved.
				while (indeterminatePlaces.size() != 0) {
					int done = 0;
					// Step 1: For each indeterminate place try to find a route
					// to the target place.
					for (Place curPlace : indeterminatePlaces) {
						int r = markRoutesToEndPlace(curPlace, markings,
								visitedPlaces, 0);
						if (r != INDETERMINATE)
							done++;

						if ((r == NOT_MARK && result == INDETERMINATE)
								|| (r == MARK)) {
							result = r;
						}
						visitedPlaces.clear();
			...
			if (result == MARK) {
				...
				if (startPlace == endPlace) {
					if ((startTrigger == ON_ENTRY) && (endTrigger == ON_EXIT)) {
						...
						log.error(formatDetailMessage(
								...
								"probe-num", Integer.toString(id), "probe.id",
								xmlId, "probe.name", name, "probe.start-place",
								startPlace.name, "probe.end-place",
								endPlace.name));
				...
				log.debug(" }");
				log.debug("probes[" + id + "].instrumentations.count = "
						+ markingCount);

			} else {
				log.error(formatDetailMessage(
						"Start and end place of probe not connected.",
						"probe-num", Integer.toString(id), "probe.id", xmlId,
						"probe.name", name, "probe.start-place",
						startPlace.name, "probe.end-place", endPlace.name));
				throw new SimQPNException();
			}
		}
	}
	 ...
	 *            - end place
	 * @param markings
	 *            - contains the result for all places. At the beginning at
	 *            least the start place should be contained in the list.
	 * @param visitedPlaces
	 *            - all places that have already been visited.
	 * @param flowLength
	 *            - the current flow length (= 0 at start)
	 * @return MARK, if a route exists, NOT_MARK, if no route exists,
	 *         INDETERMINATE, if no route can be found because of cycle.
	 * @throws SimQPNException
	 */
	private int markRoutesToEndPlace(Place end, Map<Place, Integer> markings,
			Set<Place> visitedPlaces, int flowLength) throws SimQPNException {
		// Note: The deep search starts at the end place and traverses the net
		// in direction of the start place.

		// Note: If startPlace == endPlace, the search should not return
		// immediately. It should also
		// look for a way to get from start to end in cycle including several
		// intermediary places.
		if (flowLength > 0) {
		...
		for (int i = 0; i < possibleInPlaces.length; i++) {
			// Recursion
			switch (markRoutesToEndPlace(possibleInPlaces[i], markings,
					visitedPlaces, flowLength + 1)) {
			case MARK:
				ret = MARK;
				break;
			case NOT_MARK:
	...
	/**
	 * This method backtracks all places from where tokens can be deposited in
	 * the specified place. The backtrack is only done for colors that are
	 * defined in this probe.
	 * 
	 * @param outPlace
	 *            - the place for which the possible input places should be
	 *            calculated
	 * @return array of places that are possible input places of the specified
	 *         place
	 * @throws SimQPNException
	 */
	private Place[] calculatePossibleInputPlaces(Place outPlace)
			throws SimQPNException {
		...
		for (Transition transition : outPlace.inTrans) {
			...
			placeLoop: for (Place inPlace : transition.inPlaces) {
						...
						if (c >= 0) {
							for (int m : possibleModes) {
								...
								if (tkCnt > 0) {
									...
									continue placeLoop; // The place is possible
