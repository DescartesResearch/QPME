...
public class Executor {
	 ...
	 * @exception
	 */
	public SimQPNController run(Net net, SimQPNConfiguration configuration,
			SimulatorProgress progressMonitor) throws SimQPNException {
		...
		while (clock < totRunL) {
			...
			if (beforeInitHeartBeat) {
				...
				if (((curTimeMsrm - lastTimeMsrm) >= configuration.MAX_INITIAL_HEARTBEAT)
					...
					if (SimQPNController.clock >= maxProgressInterval) {
						...
						timeBtwHeartBeats = (SimQPNController.clock / (curTimeMsrm - lastTimeMsrm))
								* progressUpdateRate;
					}
			...
			if (configuration.stoppingRule != configuration.FIXEDLEN
				...
				if (configuration.timeBtwChkStops > 0)
					nextChkAfter = SimQPNController.clock
							+ configuration.timeBtwChkStops;
				else
					nextChkAfter = SimQPNController.clock + clockTimePerSec
